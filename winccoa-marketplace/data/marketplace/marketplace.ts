...
import type { Repository, ApiError, Theme, ToastType } from "./types";
...
export class MarketplaceUI {
      ...
      if (
        ...
        const sslError = new Error(
          "SSL Certificate Error: The server uses an untrusted certificate. " +
            ...
            this.baseUrl +
            " directly, or 2) Use HTTP instead of HTTPS for local testing, or " +
  ...
  private initializeEventListeners(): void {
    ...
    document
      .getElementById("registered-projects-menu-item")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("pmon-credentials-btn")
      ?.addEventListener("click", () => {
  ...
  private async loadAllOrganizations(): Promise<void> {
    try {
      ...
      this.showToast(
        `Loaded ${this.repositories.length} repositories from all organizations`,
        "success",
      );
    } catch (error) {
  ...
  private async loadRepositories(
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.showToast(
          `Loaded ${this.repositories.length} repositories from ${organization}`,
          "success",
        );
      } else {
        ...
        this.showError(
          `Failed to load repositories from "${organization}": ${errorMsg}`,
        );
        this.repositories = [];
        // Don't render here - will be rendered after all status info is loaded
      }
      ...
      if (apiError.name !== "AbortError") {
        ...
        if (apiError.isSSLError) {
          ...
          errorMessage =
            "Network Error: " + (apiError.message || "Unknown error");
        }
  ...
  private async loadRegisteredProjects(): Promise<void> {
    try {
      ...
      if (apiError.isSSLError) {
        console.warn(
          "SSL Certificate Issue loading registered projects:",
          apiError.message,
        );
      } else if (apiError.isConnectionError) {
        console.warn(
          "Connection Issue loading registered projects:",
          apiError.message,
        );
      } else {
        console.warn(
          "Could not load registered projects:",
  ...
  private async loadLocalRepositories(): Promise<void> {
    try {
      ...
      if (response.ok) {
        const localRepos: Array<{ addon: string; fileContent: any }> =
          await response.json();
      ...
      console.warn(
        "Could not load local repositories:",
        apiError.message || "Unknown error",
      );
    }
  }
  ...
  private renderRepositoryList(): void {
    ...
    const repositoryItems = this.repositories
      .map((repo) => {
        // Check if registered by comparing subproject name (if available) or repository name
        const isRegistered = this.registeredProjects.includes(
          this.getRepositoryIdentifier(repo),
        );
        const statusClass = isRegistered
          ...
          : repo.cloned
            ? "cloned"
            : "";
        const statusTooltip = isRegistered
          ? "Installed"
          : repo.cloned
            ? "Downloaded"
            : "Not downloaded";
    ...
    items.forEach((item) => {
      ...
      if (repoData) {
        ...
        (item as HTMLElement).style.display =
          matchesSearch && matchesKeywords ? "block" : "none";
      }
    });
  }
  ...
  private updateRepositoryDetails(repo: Repository): void {
    ...
    if (visibilityChip)
      visibilityChip.setAttribute(
        "icon",
        repo.private ? "lock-filled" : "unlock-filled",
      );
    if (visibilityText)
      visibilityText.textContent = repo.private ? "Private" : "Public";
    ...
    if (repoStarsText) {
      const starsValue =
        repo.stars !== undefined && repo.stars !== null ? repo.stars : "N/A";
      repoStarsText.textContent = `${starsValue} ${repo.stars !== 1 ? "stars" : "star"}`;
    }
    if (repoForksText) {
      const forksValue =
        repo.forks !== undefined && repo.forks !== null ? repo.forks : "N/A";
    ...
    const repoCloneUrl = document.getElementById(
      "repo-clone-url",
    ) as HTMLAnchorElement;

    if (repoSize) repoSize.textContent = this.formatSize(repo.size);
    if (repoCreated)
      repoCreated.textContent = this.formatExactDate(repo.createdAt);
    if (repoUpdated)
      repoUpdated.textContent = this.formatRelativeTime(repo.updatedAt, false);
    if (repoCloneUrl) {
      if (repo.cloneUrl) {
  ...
  private async fetchAndRenderReadme(repo: Repository): Promise<void> {
    ...
    const match = repo.cloneUrl.match(
      /github\.com[/:]([\w-]+)\/([\w-]+?)(\.git)?$/,
    );
    if (!match) {
      readmeSection.style.display = "none";
    ...
    if (isRegistered) {
      ...
      if (hasMetadata) {
        ...
        registerBtn?.setAttribute(
          "title",
          "Cannot install: Repository metadata not available",
        );
      }
      ...
      registerBtn?.setAttribute(
        "title",
        "Install subproject (will download first)",
      );
      unregisterBtn?.setAttribute("disabled", "");
  ...
  private async showCloneModal(): Promise<void> {
    ...
    try {
      ...
      const defaultPath =
        Array.isArray(pathData) && pathData.length > 0
          ? pathData[0]
          : "Unknown path";
      ...
      this.showError(
        "Failed to get default download path: " + (error as Error).message,
      );
    }
  }
      ...
      loginBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (errorMessage && errorText) {
              errorText.textContent =
                "Failed to set credentials: " + response.statusText;
              errorMessage.style.display = "block";
            }
            loginBtn.removeAttribute("disabled");
            loginBtn.textContent = "Login";
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.changes !== undefined) {
          const changesText =
            data.changes === 1 ? "1 change" : `${data.changes} changes`;
    ...
    try {
      ...
      if (pmonBtn) {
        if (response.ok) {
          ...
          pmonBtn.setAttribute(
            "title",
            "PMON Credentials Set (Click to change)",
          );
        } else {
  ...
  private async checkPmonCredentials(): Promise<boolean> {
    try {
      ...
      if (response.ok) {
        ...
        this.showError(
          "Failed to check PMON credentials: " + response.statusText,
        );
        return false;
      }
    } catch (error) {
      this.showError(
        "Failed to check PMON credentials: " + (error as Error).message,
      );
      return false;
    }
  }
  ...
  private async registerSubProject(): Promise<void> {
    ...
    if (!repositoryBeingRegistered.cloned) {
      ...
      const cloneSuccess = await this.cloneForRegistration(
    ...
    if (!repositoryBeingRegistered.fileContent) {
      this.showError(
        "Cannot install: Repository metadata not available. Please download or update the repository first.",
      );
      return;
    }
   ...
   */
  private async performCloneOperation(
  ...
  private async unregisterSubProject(): Promise<void> {
    ...
    const result = await window.ixShowUnregisterConfirm?.(
  ...
  private async deleteRepository(): Promise<void> {
    ...
    const nameToCheck =
      repositoryBeingDeleted.subprojectName || repositoryBeingDeleted.name;
    const isRegistered = this.registeredProjects.includes(nameToCheck);

    if (isRegistered) {
      this.showError(
        "Cannot delete: Repository is registered as a subproject. Unregister it first.",
      );
      return;
    }

    // Show confirmation modal
    const confirmed = await window.ixShowDeleteConfirm?.(
      repositoryBeingDeleted.name,
    );
